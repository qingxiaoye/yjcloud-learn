...
class tfidfProcessor():
    ...
    # @staticmethod
    # def getWordCount(lineList, # type: List
    #                  wordLookup: # type: List
    #                  ):
    #
    #     '''This function get
    #         1. each word frequency
    #         2. each word frequency for a certain paragraph
    #         3. dictionary that mapping word to number
    #     '''
    #     countDict = defaultdict(int)
    #     word2index = dict(zip(wordLookup, range(len(wordLookup))))
    #     paracountDict = np.zeros(shape = (len(lineList), len(wordLookup)))
    #
    #     for row, line in enumerate(lineList):
    #         for word in line:
    #             countDict[word] += 1
    #             paracountDict[row][word2index.get(word)] += 1
    #
    #     return countDict, paracountDict, word2index

    def tfidfProcessor(self):
        '''
        :param corpus: ['a b c', 'd e f']
        '''
        if isinstance(self.lineList[0], List):
            corpus = [' '.join(x) for x in self.lineList]
        elif isinstance(self.lineList[0], Text):
            pass
        else:
        ...
        self.wordLookUp = tfidf.vocabulary_
        # self.tfidf_vector = res.toarray()
        self.tfidfModel = self.save_or_load(tfidf)
...
#     a.tokenizer()
#
#     b = tfidfProcessor(lineList = a.unionSentence)
#     # res = b.predictProcess(['古 时期 的 猎豹'], b.tfidfModel)
#     # print(res)
#
